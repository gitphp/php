...
//通用分类类信息
	class Page{
		 ...
		 * @return string	分类字符串
		*/
		public function getPage($uri,$rows,$page,$length){
			//计算分页信息
			...
			for($i=1;$i<=$pagetotal;$i++){
				$num_str.= "<a href='{$uri}&page={$i}'> $i &nbsp;</a>";
				
			}
			// style3,下拉框分页信息展示----加onchange事件改变当前页面的uri
			$select_str="<select onchange=\"location.href='{$uri}&page='+this.value\">";
			for($i=1;$i<=$pagetotal;$i++){
				if($page==$i){
